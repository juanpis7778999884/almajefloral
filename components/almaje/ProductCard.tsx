...
export default function ProductCard({ product, onCustomize }: ProductCardProps) {
  ...
  return (
    <article
      className={`relative organic-card p-4 transition-all duration-300 ${isHovered ? "hover-wobble" : ""}`}
    ...
    >
      ...
      <div className="relative aspect-[4/5] mb-4 overflow-hidden rounded-lg">
        ...
        <Image
          ...
          alt={product.name}
          fill
          className={`object-cover transition-all duration-500 ${isHovered ? "scale-105" : "scale-100"} ${imageLoaded ? "opacity-100" : "opacity-0"}`}
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between pt-2">
          <div>
            ...
            <span className="font-body text-xs text-[var(--muted-foreground)]">COP</span>
          </div>

          <button
            onClick={() => onCustomize(product)}
            className="relative px-5 py-2.5 bg-[var(--soft-pink)] text-[var(--foreground)] font-body font-medium text-sm transition-all hover:scale-105 hover:shadow-md group overflow-hidden"
            style={{ 
          ...
          >
            <span className="relative z-10 flex items-center gap-2">
              Personalizar
              <svg className="w-4 h-4 transition-transform group-hover:translate-x-1" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
                <path d="M7 4l6 6-6 6" />
              </svg>
            </span>
            {/* Ribbon tail effect */}
