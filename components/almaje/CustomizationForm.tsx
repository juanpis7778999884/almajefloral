...
const flowerTypes = [
  ...
  { id: "anturio", name: "Anturio", emoji: "💮" },
  { id: "gerbera", name: "Gerbera", emoji: "🌺" },
]
...
const wrapStyles = [
  ...
  { id: "rosa", name: "Papel Rosa" },
  { id: "dorado", name: "Papel con Bordes Dorados" },
  { id: "canasta", name: "Canasta Just For You" },
  { id: "caja", name: "Caja Redonda" },
]
...
// ==========================================
// ACCESORIOS - Modifica los precios aqui
// ==========================================
const accessories = [
  { id: "mariposas", name: "Mariposas Doradas", icon: "🦋", price: 500 },
  { id: "corona_pequena", name: "Corona Pequena", icon: "👑", price: 3000 },
  { id: "corona_grande", name: "Corona Grande", icon: "👑", price: 5000 },
  { id: "luces", name: "Luces LED", icon: "💡", price: 7000 },
  { id: "tarjeta", name: "Tarjeta Personalizada", icon: "💌", price: 4000 },
  { id: "perlas", name: "Perlas Decorativas", icon: "⚪", price: 2000 },
  { id: "hojas", name: "Hojas Verdes Extra", icon: "🌿", price: 1500 },
  { id: "foto", name: "Porta Foto", icon: "📷", price: 3500 },
]
// ==========================================
...
export default function CustomizationForm({ selectedProduct, isOpen, onClose }: CustomizationFormProps) {
  ...
  const handleSubmitToWhatsApp = () => {
    ...
    // Calcular accesorios con precios
    const accessoriesWithPrices = formData.accessories
      .map(accId => {
        const acc = accessories.find(a => a.id === accId)
        return acc ? `${acc.name} ($${acc.price.toLocaleString("es-CO")})` : null
      })
      .filter(Boolean)
    const accessoriesList = accessoriesWithPrices.join(", ") || "Ninguno"
    const accessoriesTotal = formData.accessories.reduce((sum, accId) => {
      const acc = accessories.find(a => a.id === accId)
...
${accessoriesTotal > 0 ? `- Total accesorios: $${accessoriesTotal.toLocaleString("es-CO")} COP` : ""}

*Informacion del Pedido:*
- Cantidad: ${formData.quantity}
- Para: ${formData.recipientName || "No especificado"}
- Ocasion: ${formData.occasion || "No especificada"}
...
Gracias por elegir AlMaje Floral!
    ...
    const whatsappUrl = `https://wa.me/3115510169?text=${encodedMessage}`
            ...
            <div>
              ...
              {formData.accessories.length > 0 && (
                <p className="mt-3 text-sm text-[var(--muted-foreground)]">
                  Total accesorios: <span className="font-medium text-[var(--primary)]">
                    ${formData.accessories.reduce((sum, accId) => {
                      const acc = accessories.find(a => a.id === accId)
                      return sum + (acc?.price || 0)
